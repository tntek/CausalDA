...
class VarianceEncoder(nn.Module):
    ...
    def __init__(self, shape, init=0.1, channelwise=False, eps=1e-5):
        ...
        # self.b = nn.Parameter(temp)
        
        self.b = nn.Parameter(torch.full(b_shape, init))
        
        # init_tensor = torch.normal(0,1,size=b_shape)
        # temp = torch.full(b_shape, init)
        # self.temp = 0.1*init_tensor + temp
        # self.temp = init_tensor


        # print(1)
    def forward(self, x):
        # init_tensor = torch.normal(0,1,size=self.shape)
        # self.temp = init_tensor
