...
class ClipTestTimeTuning(nn.Module):
    ...
    def get_text_features(self):
        text_features = []
        prompts = self.prompt_learner()#[65,77,512]
    ...
    def get_text_features_new(self,ctx_new):
        text_features = []
        prompts = ctx_new#[65,77,512]
        # torch.save(prompts, 'learned_prompts.pt')
        # torch.save(prompts, 'learned_tokenized.pt')
        tokenized_prompts = self.prompt_learner.tokenized_prompts
        t_features = self.text_encoder(prompts, tokenized_prompts)
    ...
    def inference_new(self, image,ctx_new):
        with torch.no_grad():
            image_features = self.image_encoder(image.type(self.dtype))

        text_features = self.get_text_features_new(ctx_new)
        # text_features_save = text_features.clone().detach()
        # torch.save(text_features_save, 'learned_features.pt')
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
    ...
    def inference_with_image(self, image_features):
        # with torch.no_grad():
        #     image_features = self.image_encoder(image.type(self.dtype))
    ...
    def set_text_features_wd(self, text_features):
        self.text_features_wd = text_features


    def inference_with_updated_txtfea(self, image):
        with torch.no_grad():
            image_features = self.image_encoder(image.type(self.dtype))

        text_features = self.text_features_wd
        # text_features_save = text_features.clone().detach()
        # torch.save(text_features_save, 'learned_features.pt')
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
    ...
    def inference_with_updated_imgfea(self, all_clip_feature_prompt):
        # with torch.no_grad():
            # image_features = self.image_encoder(image.type(self.dtype))
